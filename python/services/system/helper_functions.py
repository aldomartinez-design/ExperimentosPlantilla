...
def resolve_foreign_keys_bulk(model, df):
    ...
    for local_col, RefModel, ref_pk_col in fk_info:
        ...
        if local_col not in df.columns:
            continue
        df[local_col] = pd.to_numeric(df[local_col], errors="coerce").astype("Int64")
        needed_ids = (
            df[local_col]
            .dropna()
            .astype(str)
...
def base_query(model):
    ...
    # -------------------------------------------------------------------------
    if session["nombre_rol"] not in ("Administrador", "Sistema"):
        if hasattr(model, "id_usuario"):
    ...
    return query,aliased_name_columns,m2m_search_columns,joined_label_names
